...
class Miner():
    ...
    def Start(self):
        ...
        if(self.inputs):
            self.ScanTrendyolPage(urlparse(self.inputs[0]).path)
            return

        self.ScanTrendyolPage("https://www.trendyol.com/")
            ...
            for merchant in product["merchantListings"]:
                ...
                try:
                    self.Database.Execute("""
                        INSERT INTO seller (sellerID, productID, price)
                        VALUES (%s, %s, %s)
                        ON DUPLICATE KEY UPDATE price = %s
                    """, (merchantID, productID, price, price))
                except:
                    pass
...
class Urls(Miner):
    ...
    def AddNotVisitedLinks(self, urls):
        ...
        addUrlQuey = '''
            ...
            UNION ALL
            SELECT vurl FROM vurls
            WHERE vurl NOT IN (%s)
        '''
        try:
            self.Database.Execute(addUrlQuey, (",".join('%s' for _ in urls),))  # Parameter for urls list
